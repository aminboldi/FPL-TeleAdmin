...
- **The stored value wins over the environment, except at startup.** Settings are seeded from environment variables the first time the database is created, with `INSERT OR IGNORE`; after that the row is authoritative so the dashboard can change it. That meant editing `.env` and redeploying did nothing at all, silently — a `PRICE_PREDICTIONS_ENABLED=false` seeded during the off-season stayed false forever, and no amount of correcting `.env` reached it. `_apply_env_overrides()` now updates the row at startup from any variable that is **explicitly set and non-empty**, and logs the replacement. A variable the environment does not set leaves the stored value alone, so `/set` still owns everything else.
